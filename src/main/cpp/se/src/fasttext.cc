...
void FastText::findNNSent(const Matrix& sentenceVectors, const Vector& queryVec,
                          int32_t k, const std::set<std::string>& banSet, int64_t numSent, 
                          const std::vector<std::string>& sentences, std::vector<std::pair<real,std::string>>& predictions) {
  real queryNorm = queryVec.norm();
  if (std::abs(queryNorm) < 1e-8) {
    queryNorm = 1;
  }
  ...
  while (i < k && heap.size() > 0) {
    auto it = banSet.find(heap.top().second);
    if (!std::isnan(heap.top().first)) {
      predictions.push_back(std::make_pair(heap.top().first, heap.top().second ));
      std::cout << heap.top().first << " " 
                                    << heap.top().second << " " 
                                    << std::endl;
      i++;
...
void FastText::nnSent(std::istream& in, int32_t k, std::string filename, std::vector<std::pair<real, std::string>>& predictions) {  
  std::string sentence;
  std::ifstream in1(filename);
  int64_t n = 0;
  ...
  std::set<std::string> banSet;
    ...
    findNNSent(sentenceVectors, query, k, banSet, n, sentences, predictions);
    std::cout << std::endl;
