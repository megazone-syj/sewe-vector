...
namespace SEWrapper {
    ...
    std::vector<std::pair<real,std::string>> FastTextApi::nnSent(
            const std::string& filename,const std::string& text, int32_t k) {
        std::vector<std::pair<real,std::string>> predictions;
        std::istringstream in(text);
        fastText.nnSent(in, k,filename, predictions);
        return predictions;
    }
