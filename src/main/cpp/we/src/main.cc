...
void printUsage() {
  std::cerr
	...
	<< "  nnSentProb                query for nearest neighbors for sentences\n"
...
void nnSentProb(int argc, char** argv) {
  int32_t k;
  if (argc == 4) {
    k = 10;
  } else if (argc == 5) {
  ...
  std::ifstream in1;
  fasttext.nnSentProb(k,std::string(argv[3]),in1,predictions );
  exit(0);
}
...
int main(int argc, char** argv) {
  ...
  if (command == "skipgram" || command == "cbow" || command == "supervised" ||
    ...
    nnSent(argc, argv);
  } else if (command == "nnSentProb") {
    nnSentProb(argc, argv);  } else if (command == "analogies") {
    analogies(argc, argv);
