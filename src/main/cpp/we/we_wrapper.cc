...
namespace WEWrapper {
    ...
    std::vector<std::pair<real,std::string>> FastTextApi::nnSentProb(
            int32_t k,const std::string& filename,
            const std::string& text) {
        std::vector<std::pair<real,std::string>> predictions;
        std::istringstream in(text);
        fastText.nnSentProb(k,filename, in, predictions);
        return predictions;
    }
